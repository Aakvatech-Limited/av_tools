...
def load_json(file):
	...
	with open(json_file_path) as f:  # nosemgrep: frappe-security-file-traversal - path is app-owned and fixed
		return json.load(f)
...
def create_fields_from_json(custom_fields_obj):
	...
	for custom_field in custom_fields_obj:
		...
		custom_field_dict = {field_name: custom_field.get(field_name) for field_name in field_list}
		...
		if module_name and not frappe.db.exists("Module Def", module_name):
			frappe.get_doc(
				{"doctype": "Module Def", "module_name": module_name, "app_name": "av_tools"}
