...
def analyze_doctype_with_openai(doctype_name, prompt, doc_data=None):
	...
	def send_message_to_openai(user_message, log_title, log_details=""):
		...
		frappe.log_error(
			title=log_title, message=f"{log_details} \n <br> {response.choices[0].message.content}"
		)
	...
	try:
		if doc_data:
			...
			send_message_to_openai(
				user_message, "OpenAI Doc Data Sent", f"Successfully sent doc_data for {doctype_name}"
			)
			...
			for i, content_block in enumerate(content_blocks, 1):
				...
				send_message_to_openai(
					file_message, "OpenAI File Sent", f"Successfully sent file {i} for {doctype_name}"
				)
		...
		frappe.log_error(title="OpenAI API Call Failed", message=frappe.get_traceback())
		frappe.throw(title="AI Assist Error", msg=str(e))
