...
const distribute_net_weight = (frm, netWeight) => {
	...
	const totalQty = items.reduce(
		(sum, row) => sum + flt(row.custom_requested_qty || row.qty || 0),
		0
	);
	...
	items.forEach((row) => {
		...
		const kgQty = useProportional ? (baseQty / totalQty) * flt(netWeight) : perItem;
...
const set_net_weight = (frm) => {
	if (
		frm.doc.tare_weight != null &&
		frm.doc.tare_weight !== "" &&
		frm.doc.gross_weight != null &&
...
const add_create_buttons = (frm) => {
	...
	const targets = CREATE_TARGETS_BY_SOURCE[frm.doc.document_type] || CREATE_TARGET_DOCTYPES;
...
const apply_reference_party = (frm, referenceData) => {
	...
	const isPurchaseDoc = ["Purchase Order", "Purchase Invoice", "Purchase Receipt"].includes(
		frm.doc.document_type
	);
...
const parse_valpoids = (xmlText) => {
	const match = xmlText.match(/<id>ValPoids<\/id><value>\s*([^<]+)<\/value>/i);
	if (!match) {
		...
		call_url(frm._read_weight_url)
			...
			.then((result) => {
				...
				if (!data || data.weight == null) {
					frappe.msgprint(
						__("Missing weight in response from {0} (HTTP {1}).", [
							result.url,
							result.status,
						])
					);
				...
				Promise.resolve(frm.set_value(target_field, data.weight))
					...
					.then(() => {
						...
						frappe.show_alert(
							{
								message: __("{0} captured: {1}", [
									label,
									format_number(data.weight),
								]),
								indicator: "green",
							},
							5
...
const apply_vehicle_tare = (frm) => {
	...
	frappe.db.get_value("Vehicle", frm.doc.vehicle, "default_tare_weight").then((r) => {
		const weight = flt(r && r.message ? r.message.default_tare_weight : null);
		if (!weight) {
			frappe.show_alert({
				message: __("Selected Vehicle has no Default Tare Weight."),
				indicator: "orange",
			});
			return;
		}
