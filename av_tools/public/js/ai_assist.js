$(document).on("app_ready", function () {
	var registered_doctypes = {};
	frappe.router.on("change", () => {
		var route = frappe.get_route();
		if (route && route[0] == "Form" && !registered_doctypes[route[1]]) {
			registered_doctypes[route[1]] = true;
						...
						frappe.call({
							...
							error: function (r) {
								...
								dialog.set_value(
									"response",
									__(
										"Error occurred while analyzing. Please check the console for details."
									)
								);
