...
function get_active_form() {
	return window.cur_page?.page?.frm || null;
}

$(document).on("form-refresh", function (e, frm) {
	setTimeout(function () {
		setup_approver_buttons(frm);
	}, 50);
});
...
function setup_approver_buttons(frm) {
	...
	if (
		frm.custom_buttons &&
		(frm.custom_buttons[__("Mark as Approved")] ||
	...
	var is_expired =
		...
		frappe.datetime.get_diff(frappe.datetime.now_date(), approver_row.expiry_date) > 0;
	if (is_expired && current_user === approver_row.approver) {
		if (approver_row.delegate_to) {
			frappe.show_alert({
				message: __(
					...
					[
						frappe.datetime.str_to_user(approver_row.expiry_date),
		...
		frappe.show_alert({
			message: __(
				"Your approval authority for this document expired on {0}, but no delegate is set, so you may still act.",
				[frappe.datetime.str_to_user(approver_row.expiry_date)]
			),
			indicator: "orange",
		});
