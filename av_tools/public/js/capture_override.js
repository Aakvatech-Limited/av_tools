(function () {
	...
	function requestCaptureSettings() {
		if (captureSettingsPromise) {
		...
		captureSettingsPromise = new Promise(function (resolve) {
			frappe.call({
				...
				callback: function (response) {
					captureSettingsCache = normalizeCaptureSettings(
	...
	function getCachedCaptureSettings(capture) {
		if (capture && capture.__av_tools_capture_settings) {
			return Promise.resolve(capture.__av_tools_capture_settings);
		}

		if (captureSettingsCache) {
			var settings = normalizeCaptureSettings(captureSettingsCache);
			if (capture) {
				capture.__av_tools_capture_settings = settings;
			}
	...
	function getCaptureSettings(capture, forceRefresh) {
		if (!forceRefresh) {
			return getCachedCaptureSettings(capture).then(function (settings) {
				if (settings) {
					return settings;
				}
				return requestCaptureSettings().then(function (freshSettings) {
					if (capture) {
						capture.__av_tools_capture_settings = freshSettings;
					}
		...
		return requestCaptureSettings().then(function (settings) {
			if (capture) {
				capture.__av_tools_capture_settings = settings;
			}
			return settings;
	...
	function warmCaptureSettingsCache() {
		requestCaptureSettings().catch(function () {
			return null;
		});
	}
	...
	function buildConstraints(capture, settings) {
		return {
			video: buildTrackConstraints(capture, settings),
			audio: false,
		};
	}
	...
	function applyTrackConstraints(track, capture, settings) {
		...
		return track
			.applyConstraints(buildTrackConstraints(capture, settings))
	...
	function installCaptureOverride() {
		...
		proto.show = function () {
			...
			if (frappe.is_mobile()) {
				var cachedSettings =
					me.__av_tools_capture_settings ||
					(captureSettingsCache && normalizeCaptureSettings(captureSettingsCache));

				if (cachedSettings) {
					me.__av_tools_capture_settings = cachedSettings;
				}
		...
		proto.render_stream = function () {
			...
			return getCaptureSettings(me).then(function (settings) {
				if (!settings.enabled) {
		...
		proto.setup_take_photo_action = function () {
			var me = this;
			var settings =
				me.__av_tools_capture_settings || normalizeCaptureSettings(defaultCaptureSettings);
	...
	function retryInstall() {
		...
		var interval = setInterval(function () {
			...
			if (installCaptureOverride() || attempts > 50) {
				if (frappe.ui && frappe.ui.Capture) {
					warmCaptureSettingsCache();
				}
