(() => {
  ...
  (function() {
    ...
    function retry_installer(installer, interval_key) {
      if (installer() || av_tools.financial_statements[interval_key])
        return;
      let attempts = 0;
    ...
    function install_accounts_receivable_summary_override() {
      ...
      report.formatter = function(value, row, column, data, default_formatter, filter) {
        if (column.fieldname === "party") {
          if (data && data.party_type === "Customer" && data.party) {
            column.link_onclick = "av_tools.financial_statements.open_customer_general_ledger(" + JSON.stringify({ party_type: data.party_type, party: data.party }) + ")";
          } else {
  ...
  // ../av_tools/av_tools/public/js/capture_override.js
  (function() {
    var captureSettingsRequest;
    function getCaptureSettings() {
      if (!captureSettingsRequest) {
        captureSettingsRequest = frappe.call("av_tools.av_tools_hooks.capture.get_capture_settings").then(function(response) {
...
//# sourceMappingURL=av_tools.bundle.B4KQJMCA.js.map
