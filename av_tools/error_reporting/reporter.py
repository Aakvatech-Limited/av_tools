...
SECRET_RE = re.compile(r"(?i)(password|passwd|api[_-]?key|api[_-]?secret|token|authorization|cookie|session)(\s*[:=]\s*)([^\s,;]+)")
...
def build_payload(report_date):
    ...
    for row in logs:
        ...
        if not observation:
            continue
        key = (
            observation["app_name"], observation["file_path"], observation.get("function_name"),
            observation.get("line_number"), observation["exception_type"],
        )
...
def send_payload(payload):
    ...
    url = settings.central_server_url.rstrip("/") + "/api/method/aakvatech_error_registry.api.intake.report_errors"
...
def _get_git_sha(app_name):
    try:
        ...
        return subprocess.check_output(["git", "-C", app_path, "rev-parse", "HEAD"], text=True, timeout=5).strip()
    except Exception:
        return None
