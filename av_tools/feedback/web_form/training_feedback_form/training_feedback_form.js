	...
	function initializeForm() {
        ...
        ">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <label for="${fieldId}" style="
              ...
              color: #2e3a59;
            ">
              <span style="color: #5e6c84;">Q${index + 1}:</span> ${question.question}
      ...
      `;
		...
		function createClearButton(fieldType, index) {
			if (["Multiple Choice", "Rating", "Checkboxes", "Dropdown"].includes(fieldType)) {
				return `
		...
		function createQuestionField(question, index, isRequired, fieldId) {
			switch (question.fieldtype) {
				case "Multiple Choice":
					return createMultipleChoiceField(question, index, isRequired, fieldId);
				case "Paragraph":
		...
		function createMultipleChoiceField(question, index, isRequired, fieldId) {
          ...
          <div class="form-check">
            <input class="form-check-input" type="radio"
              name="q${index}" id="${fieldId}-${trimmedOpt.replace(/\s+/g, "-")}"
              value="${trimmedOpt}" ${isRequired}>
            <label class="form-check-label" for="${fieldId}-${trimmedOpt.replace(/\s+/g, "-")}">
              ${trimmedOpt}
            </label>
          </div>
        `;
      ...
      `;
		...
		function createRatingField(question, index, isRequired) {
			const maxRating = question.options ? parseInt(question.options.trim()) || 5 : 5;
			return `
      ...
      `;
		...
		function setupCheckboxesClear(btn, index) {
			...
			checkboxes.forEach((cb) => cb.addEventListener("change", updateClearButton));
