...
@frappe.whitelist()
def get_salary_structure_components(salary_structure: str) -> dict[str, Any]:
...
def run_calculation(
	...
	calculate_based_on: str,
	gross_pay: float | int = 0,
	net_pay: float | int = 0,
	selected_components: list[str] | str | None = None,
	earning_overrides: dict[str, float] | str | None = None,
...
def get_salary_slip_preview(
	...
	gross_pay: float | int,
	net_pay: float | int,
	earnings_data: list[dict[str, Any]] | str,
	deductions_data: list[dict[str, Any]] | str,
	employee: str | None = None,
) -> str:
	...
	return frappe.render_template(  # nosemgrep: frappe-ssti - fixed app template path, not user input
...
def create_salary_structure_assignment(
	employee: str, salary_structure: str, from_date: str, base: float | int = 0
) -> str:
