...
from frappe.utils import flt, get_fullname, get_url_to_form, nowdate
...
def validate_item(item, method):
	...
	if disabled == 1:
		msg_throw(_(f"Item: {frappe.bold(item)} is disabled"), method)


@frappe.whitelist()
def validate_price_list(price_list, method):
	if not frappe.get_cached_value("Price List", price_list, "enabled"):
		msg_throw(_(f"Price List: {frappe.bold(price_list)} is disabled"), method)


def msg_throw(msg, method="throw", alert=True):
	if method == "validate":
...
def validate_zero_prices(row):
	...
	if not row.new_price:
		frappe.throw(
			f"Zero price is not allowed for an item: {frappe.bold(row.item_code)}, row: #{frappe.bold(row.idx)}"
		)
...
def set_new_price_item(row, date):
	...
	if len(item_price) == 0:
		...
		if record.get("name"):
			...
			frappe.msgprint(
				f"New Item Price: <a href='{url_to_item_price}'>{frappe.bold(record.name)}</a> is created"
			)
		...
		frappe.throw(f"{msg}<br> Please delete one of them or set valid from and valid to date correctly")
