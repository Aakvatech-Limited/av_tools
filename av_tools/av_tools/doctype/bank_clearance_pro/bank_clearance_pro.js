...
frappe.ui.form.on("Bank Clearance Pro", {
	...
	get_payment_entries: function (frm) {
		...
		frappe.call({
			method: "av_tools.av_tools.doctype.bank_clearance_pro.bank_clearance_pro.get_balance_on",
			args: {
		...
		return frappe.call({
			...
			callback: function (r, rt) {
				frm.refresh_field("payment_entries");
				frm.refresh_fields();
				$(frm.fields_dict.payment_entries.wrapper)
					.find("[data-fieldname=amount]")
					.each(function (i, v) {
						if (i != 0) {
	...
	update_fields: function (frm) {
		...
		frm.set_value(
			"opening_difference",
			frm.doc.opening_balance - frm.doc.statement_opening_balance
		);
		frm.set_value(
			"closing_difference",
			frm.doc.closing_balance - frm.doc.statement_closing_balance
		);
	},
});
