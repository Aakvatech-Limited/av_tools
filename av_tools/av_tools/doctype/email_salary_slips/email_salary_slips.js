...
frappe.ui.form.on("Email Salary Slips", {
	...
	setup: function (frm) {
		frm.set_query("payroll_entry", function () {
			return {
				filters: [["Payroll Entry", "docstatus", "=", "1"]],
			};
		});
	},
	...
	payroll_entry: function (frm) {
		...
		if (payroll_entry) {
			frappe
				.call({
					method: "av_tools.av_tools_hooks.payroll.validate_payroll_entry_field",
	...
	get_employees: function (frm) {
		...
		if (payroll_entry) {
			frappe
				.call({
					method: "av_tools.av_tools_hooks.payroll.get_payroll_employees",
					args: { payroll_entry: payroll_entry },
				})
				.done((r) => {
					frm.doc.employees = [];
