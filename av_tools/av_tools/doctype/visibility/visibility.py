...
from frappe.utils import add_to_date, nowdate, parse_val
...
""")
	...
	def send(self, doc):
		...
		if self.set_property_after_alert:
			...
			if allow_update:
				doc.db_set(self.set_property_after_alert, self.property_value, update_modified=False)
...
def run_visibility(doc, method):
	...
	def _evaluate_alert(alert):
		# mark first: v16 db_set() fires before_change/on_change, which re-enters this hook
		if alert.name in doc.flags.vis_notifications_executed:
			return
...
def trigger_notifications(doc, method=None):
	...
	if method == "daily":
		doc_list = frappe.get_all(
			"Visibility", filters={"event": ("in", ("Days Before", "Days After")), "enabled": 1}
		)
		for d in doc_list:
...
def evaluate_alert(doc, alert, event):
	...
	try:
		if isinstance(alert, str):
			if not frappe.db.exists("Visibility", alert):
				# stale cache entry (alert deleted or rolled back): drop it and move on
				frappe.cache.hdel("vis_notifications", doc.doctype)
				return
		...
		if event == "Value Change" and not doc.is_new():
			try:
				...
				if frappe.db.is_missing_column(e):
					...
					frappe.log_error(f"Notification {alert.name} has been disabled due to missing field")
					return
				else:
					raise
