...
class SpecialClosingBalance(Document):
	...
	def on_submit(self):
		...
		if frappe.db.get_single_value("AV Tools Settings", "is_manufacture"):
			for item_row in self.closing_balance_details:
				if item_row.item:
					item_bom_list = frappe.get_all(
						"BOM", {"is_active": 1, "is_default": 1, "item": item_row.item, "docstatus": 1}
					)
					if len(item_bom_list) > 0:
						item_bom = item_bom_list[0].name
						frappe.msgprint(
							_("BOM no {0} for item {1} selected").format(item_bom, item_row.item), alert=True
						)
					else:
					...
					if item_row.quantity != item_balance:
						...
						try:
							...
							if stock_entry_doc:
								...
								frappe.msgprint(
									f"Stock Entry Created <a href='{url}'>{stock_entry_doc.name}</a>"
								)
						except Exception:
							frappe.throw(_("Stock Entry Creation Failed for item {0}").format(item_row.item))
...
def get_items(warehouse, posting_date, posting_time, company):
	...
	res = []
	for d in items:
		stock_bal = get_stock_balance(d[0], d[3], posting_date, posting_time, with_valuation_rate=True)
