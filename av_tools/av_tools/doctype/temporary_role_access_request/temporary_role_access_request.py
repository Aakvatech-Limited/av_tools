...
class TemporaryRoleAccessRequest(Document):
	...
	def before_cancel(self):
		...
		if doc_before:
			...
			if old_status not in allowed_statuses:
				frappe.throw(
					_("Only requests with status: {0} can be cancelled").format(", ".join(allowed_statuses))
				)
	...
	def _validate_role_requested(self):
		...
		if not frappe.db.exists("Role", self.role_requested):
			frappe.throw(_("Role {0} does not exist").format(frappe.bold(self.role_requested)))
	...
	def _grant_role(self):
		...
		self._send_notification(
			...
			message=_("The role {0} has been granted to {1} from {2} to {3}.").format(
				frappe.bold(self.role_requested),
				frappe.bold(self.requested_for),
				self.from_datetime,
				self.to_datetime,
	...
	def _revoke_role(self, reason="expired"):
		...
		if reason == "cancelled":
			...
			message = _("The temporary access to role {0} for {1} has expired at {2}.").format(
				frappe.bold(self.role_requested),
				frappe.bold(self.requested_for),
				now,
			)
	...
	def _notify_approvers_of_new_request(self):
		...
		self._send_notification(
			...
			message=_(
				"User {0} has requested temporary access to role {1} from {2} to {3}.<br><br>Reason: {4}"
			).format(
	...
	def _send_notification(self, recipients, subject, message):
		...
		try:
			...
			frappe.log_error(
				message=f"Failed to send notification for {self.name}: {e!s}",
...
def _send_status_notification(doc, action_label, message):
	...
	try:
		...
		frappe.log_error(
			message=f"Failed to send status notification for {doc.name}: {e!s}",
			title="Temporary Role Access - Notification Error",
		)
...
def process_temporary_role_access():
	...
	for name in approved_requests:
		try:
			...
			frappe.db.rollback()
			frappe.log_error(
				message=f"Failed to activate role for request {name}: {e!s}",
	...
	for name in active_requests:
		try:
			...
			frappe.db.rollback()
			frappe.log_error(
				message=f"Failed to expire role for request {name}: {e!s}",
