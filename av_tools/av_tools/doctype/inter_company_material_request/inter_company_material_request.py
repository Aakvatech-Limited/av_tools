...
class InterCompanyMaterialRequest(Document):
	def before_submit(self, warehouse=None):
		...
		for item in self.items_child:
			valuation_rate_data = get_valuation_rate(
				item.item_code, self.from_company, self.default_from_warehouse
			)
			if (
				not valuation_rate_data
				or valuation_rate_data.valuation_rate is None
				or valuation_rate_data.valuation_rate == 0
			):
				frappe.throw(
