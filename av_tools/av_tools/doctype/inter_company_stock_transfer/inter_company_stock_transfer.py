...
class InterCompanyStockTransfer(Document):
	def validate(self):
		allow_inter_company_stock_transfer = frappe.db.get_single_value(
			"AV Tools Settings", "allow_inter_company_stock_transfer"
		)
		if not allow_inter_company_stock_transfer:
			frappe.throw(
				"<b><h4>Inter Company Stock Transfer is not enabled, please contact system administrator</h4></b>"
			)
