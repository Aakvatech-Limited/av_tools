...
class TestTheme(IntegrationTestCase):
	def test_validate_writes_theme_css_to_av_tools_public_path(self):
		doc = frappe.get_doc({"doctype": "Theme", "theme": "body { color: red; }"})

		with (
			patch("av_tools.av_tools.doctype.theme.theme.Path.mkdir") as mock_mkdir,
			patch("av_tools.av_tools.doctype.theme.theme.Path.write_text") as mock_write_text,
		):
			doc.validate()
