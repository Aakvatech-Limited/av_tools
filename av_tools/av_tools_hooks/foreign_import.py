...
def create_import_tracker(doc, method):
    ...
    existing = frappe.db.exists(
        "Foreign Import Transaction", {"purchase_invoice": doc.name}
    )
    if existing:
        return
    ...
    try:
        ...
        frappe.db.set_value(
            "Purchase Invoice", doc.name, "foreign_import_tracker", import_tracker.name
        )

        frappe.msgprint(
            _("Foreign Import Transaction {0} created successfully").format(
                import_tracker.name
            ),
            alert=True,
        )
        ...
        frappe.log_error(
            f"Error creating Foreign Import Transaction for {doc.name}: {str(e)}"
        )
...
def cancel_import_tracker(doc, method):
    ...
    tracker_name = frappe.db.get_value(
        "Foreign Import Transaction", {"purchase_invoice": doc.name}, "name"
    )
            ...
            if tracker_name:
                try:
                    tracker_doc = frappe.get_doc(
                        "Foreign Import Transaction", tracker_name
                    )

                    # Check if LCV already added
                    existing_lcv = any(
                        row.landed_cost_voucher == doc.name
                        for row in tracker_doc.landed_cost_vouchers
                    )
                    if not existing_lcv:
                    ...
                    frappe.log_error(
                        f"Error linking LCV {doc.name} to tracker {tracker_name}: {str(e)}"
                    )
    ...
    for tracker in trackers:
        try:
            ...
            tracker_doc.exchange_differences = [
                ...
                if not (
                    row.reference_type == "Landed Cost Voucher"
                    and row.reference_name == doc.name
                )
            ]

            tracker_doc.flags.ignore_validate_update_after_submit = True
            tracker_doc.save()

        except Exception as e:
            frappe.log_error(
                f"Error unlinking LCV {doc.name} from tracker {tracker.name}: {str(e)}"
            )
...
def link_payment_to_import_tracker(doc, method):
    ...
    for tracker_data in trackers:
        ...
        if doc.paid_to_account_currency == tracker_data.currency:
            try:
                tracker_doc = frappe.get_doc(
                    "Foreign Import Transaction", tracker_data.name
                )

                # Check if payment already added
                existing_payment = any(
                    row.payment_entry == doc.name for row in tracker_doc.payments
                )
...
def unlink_payment_from_import_tracker(doc, method):
    ...
    tracker_name = frappe.db.get_value(
        "Payment Entry", doc.name, "foreign_import_tracker"
    )
...
def create_exchange_difference_je(
    tracker_doc, amount, gain_loss_type, reference_doc, description
):
    ...
    if gain_loss_type == "Gain":
        debit_account = get_supplier_payable_account(
            tracker_doc.supplier, tracker_doc.company
        )
        credit_account = (
            settings.default_exchange_gain_account
            or get_exchange_gain_loss_account(tracker_doc.company)
        )
    else:
        debit_account = (
            settings.default_exchange_loss_account
            or get_exchange_gain_loss_account(tracker_doc.company)
        )
        credit_account = get_supplier_payable_account(
            tracker_doc.supplier, tracker_doc.company
        )
...
def get_supplier_payable_account(supplier, company):
    ...
    if not payable_account:
        ...
        payable_account = frappe.db.get_value(
            "Supplier", supplier, "payable_account"
        ) or frappe.db.get_value("Supplier", supplier, "default_payable_account")
    ...
    try:
        ...
        if payment_doc.payment_type != "Pay":
            debug_info["issues"].append(
                f"Payment type is '{payment_doc.payment_type}', should be 'Pay'"
            )

        if payment_doc.party_type != "Supplier":
            debug_info["issues"].append(
                f"Party type is '{payment_doc.party_type}', should be 'Supplier'"
            )

        if payment_doc.docstatus != 1:
            debug_info["issues"].append(
                f"Payment Entry not submitted (docstatus = {payment_doc.docstatus})"
            )
        ...
        if payment_doc.party_type == "Supplier":
            ...
            for tracker in trackers:
                tracker_info = {
                    ...
                    "currency_match": payment_doc.paid_to_account_currency
                    == tracker.currency,
                    "status_ok": tracker.status in ("Active", "Draft")
                    and tracker.docstatus == 1,
                    "issues": [],
                }
                ...
                tracker_doc = frappe.get_doc("Foreign Import Transaction", tracker.name)
                already_linked = any(
                    row.payment_entry == payment_doc.name
                    for row in tracker_doc.payments
                )
                if already_linked:
                    tracker_info["issues"].append(
                        "Payment already linked to this tracker"
                    )
...
def manually_link_payment_to_tracker(payment_entry_name, tracker_name=None):
    ...
    try:
        ...
        # Check if already linked
        existing_payment = any(
            row.payment_entry == payment_doc.name for row in tracker_doc.payments
        )
        if existing_payment:
        ...
        return {
            "success": f"Payment {payment_doc.name} successfully linked to tracker {tracker_doc.name}"
        }
